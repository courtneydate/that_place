...
| BillingAccountTariffAssignment | belongs to BillingAccount; refs ReferenceDataset | id, billing_account_id, stream_id (nullable — null = all billing-role streams on the account), dataset_id, dimension_filter (JSONB), version (nullable string — null = latest active), applies_to_role (string, blank default — billing leg this tariff prices; mirrors Stream.billing_role values, e.g. `consumption_from_solar` for the solar-allocated leg and `consumption` for the grid leg of an embedded-network tenant invoice; blank = applies regardless of leg), effective_from (date), effective_to (date, nullable) |
...
| SolarAllocationRecord | belongs to BillingRun + BillingAccount (the child) | id, billing_run_id, billing_account_id, interval_start (UTC), allocated_kwh (numeric), pool_kwh (numeric — the allocatable solar pool for the interval; Σ allocated_kwh across children equals this exactly), child_grid_import_kwh (numeric — the pro-rata weight), allocation_method (pro_rata_consumption/equal_share/fixed_proportion); unique (billing_run_id, billing_account_id, interval_start) |
