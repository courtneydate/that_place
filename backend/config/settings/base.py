...
CELERY_BEAT_SCHEDULE = {
    ...
    'poll-system-feeds': {
        ...
        'schedule': 60.0,  # every 60 seconds — tasks self-throttle per provider interval
    },
    'poll-tenant-subscriptions': {
        'task': 'feeds.poll_tenant_subscriptions',
        'schedule': 60.0,  # every 60 seconds
    },
    'evaluate-reference-value-rules': {
...
# Backend subscriber account — used by the ingestion pipeline (start_mqtt).
...
# Admin account — used by MQTTCredentialService to manage per-device ACLs
# via the Mosquitto Dynamic Security plugin ($CONTROL topic).
# Must match MQTT_ADMIN_USERNAME / MQTT_ADMIN_PASSWORD in the broker bootstrap.
MQTT_ADMIN_USERNAME = env('MQTT_ADMIN_USERNAME', default='')
MQTT_ADMIN_PASSWORD = env('MQTT_ADMIN_PASSWORD', default='')

# PKI — CA key and certificate for issuing per-device mTLS client certificates.
# Values are base64-encoded PEM strings. In dev, these are auto-generated by
# the Mosquitto entrypoint script and printed to the container logs on first
# start. In production, generate offline and store in a secret manager.
